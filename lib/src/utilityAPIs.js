...
utilityAPIs.prototype = {
  ...
  getVersion: function (token, next) {
    var options = helper.getCertificateConfiguration(config);

    options.url = config.serviceBaseUrl + appVersionAPI;
    options.method = 'GET';
    options.headers = {Authorization: 'Bearer ' + token};
    request(options, function (error, response, body) {
      if (error) {
        next({
          message: error.message ? error.message : "Could not get version of application. Please contact the administrator for help."
        }, null);
      }
      else if (response.statusCode !== 200) {
        next({message: body.error, statusCode: response.statusCode}, null);
