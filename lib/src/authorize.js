...
authorize.prototype = {
  ...
  getAllResourcesOfUser: function (token, data, next) {
    request({
      url: config.serviceBaseUrl + userResourceApi + "/" + data.userId,
      method: 'GET',
  ...
  getResourceDataByResourceId: function (token, data, next) {
    request({
      url: config.serviceBaseUrl + resourcesApi + "/" + data.resourceId,
      method: 'GET',
      headers: {Authorization: 'Bearer ' + token},
    }, function (error, response, body) {
      if (error) {
  ...
  checkAccess: function (token, accessData, next) {
    request({
      url: config.serviceBaseUrl + checkAccessAPI,
      method: 'POST',
      headers: {Authorization: 'Bearer ' + token},
      json: accessData
    }, function (error, response, body) {
...
function getCertificateConfiguration() {
  ...
  if (config.requestOptions) {
    ...
    if (config.requestOptions.key && config.requestOptions.cert) {
      options.key = config.requestOptions.key;
